...
library(plotKML)
library(sp)
timeslot<-read.table("TowerActivitySlot.txt",header=FALSE,sep=",")
colnames(timeslot)<-c("towerid","weekday","weekend")
nrow(timeslot)
View(timeslot)
geo<-read.table("RMduplicateTower.txt",header=FALSE,sep=",")
nrow(geo)
View(geo)
nrow(timeslot)
...
{
  timeslot[i,4]=geo[timeslot[i,1]==geo[,1],2]
  timeslot[i,5]=geo[timeslot[i,1]==geo[,1],3]  
}

View(timeslot)
write.csv(timeslot,"TowerActSlotGeo.csv")
